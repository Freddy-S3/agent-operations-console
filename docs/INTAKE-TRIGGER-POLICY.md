...
- The ticket has the required ready status, label, or equivalent provider signal.
- The ticket belongs to the agreed ticket class.
...
Sprint membership is therefore necessary but not sufficient.
...
| A ticket is added to the current sprint. | Create an Intake Candidate and revalidate it after the debounce window. | Sprint scope signals near-term intent without assuming the ticket is ready or safe. |
...
| A bulk sprint assignment makes many tickets eligible at once. | Admit candidates through workspace queue, concurrency, ticket-class, and cost limits. | Sprint scope defines eligibility, while capacity controls prevent a thundering herd. |
...
The next implementation should add normalized sprint and relationship fields to the provider-neutral work-item shape, then introduce an Intake Candidate layer before changing `ingestJiraWebhook` into a live trigger.
...
The first live rollout should enable one workspace, one Jira project, one repository, one ticket class, one trigger profile, and one operator-approved runner.
...
- Which exact Jira status and label combination means ready for the first paid pilot.
